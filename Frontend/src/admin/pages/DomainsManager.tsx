...
import {
  ...
  Bot, Shield, Terminal, Cpu, Rocket, Gamepad2,
} from "lucide-react";
...
export default function DomainsManager() {
  ...
  const fetchDomains = async () => {
    setLoading(true);
    try {
      const r = await adminApi.getAllDomains();
      setDomains(r.data);
    } catch {}
    finally {
      setLoading(false);
  ...
  const save = async () => {
    setSaving(true);
    try {
      if (editDomain) await adminApi.updateDomain(editDomain._id, form);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-orbitron font-black text-2xl sm:text-3xl text-white tracking-wider">
            ABOUT TRACKS / DOMAINS
          </h1>
          <p className="text-slate-400 font-space text-sm mt-1">
            {domains.length} tracks shown in the public About section.
          </p>
        </div>
      ...
      <div className="glass rounded-3xl overflow-x-auto border-cyan-500/20 shadow-xl">
        {loading ? (
          ...
          <table className="data-table min-w-full">
            ...
            <tbody>
              {domains.map((d) => {
                const Icon = ICON_MAP[d.icon] || Bot;
                return (
                  <tr key={d._id}>
                    <td>
                      <div
                        className="w-10 h-10 rounded-xl flex items-center justify-center border"
                        style={{ background: `${d.color}15`, borderColor: `${d.color}35` }}
                      >
                        <Icon className="w-5 h-5" style={{ color: d.color }} />
                      </div>
                    </td>
                    <td className="font-bold text-white">{d.title}</td>
                    <td className="text-sm text-cyan-300 font-space">{d.badge}</td>
                    <td className="font-mono-matrix text-xs text-slate-400">{d.order}</td>
                    <td>
                      <span className={d.isActive ? "text-emerald-400" : "text-slate-600"}>
                        {d.isActive ? "● LIVE" : "○ OFF"}
                      </span>
                    </td>
                    <td>
          ...
          >
            ...
            >
              ...
              <p className="text-slate-300 text-sm font-space mb-6">
                This will remove it from the public About section immediately.
              </p>
              ...
              <div className="p-8">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-800">
                  <h2 className="font-orbitron font-black text-xl text-white tracking-wider flex items-center gap-2">
                    ...
                    <span>{editDomain ? "EDIT TRACK" : "ADD NEW TRACK"}</span>
                  </h2>
                ...
                <div className="space-y-5">
                  ...
                  <FormField label="Track Title" value={form.title} onChange={(v) => updateField("title", v)} />
                  <FormField label="Description" value={form.desc} onChange={(v) => updateField("desc", v)} textarea />
                  <FormField label="Badge (e.g. TRACK 01)" value={form.badge} onChange={(v) => updateField("badge", v)} />
                  ...
                  <div className="pt-6">
                    ...
                    >
                      ...
                      <span>{saving ? "SAVING..." : editDomain ? "UPDATE TRACK" : "ADD TRACK"}</span>
                    </button>
                  </div>
                </div>
              </div>
